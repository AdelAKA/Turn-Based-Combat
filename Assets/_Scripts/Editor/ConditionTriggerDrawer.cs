...
{
    [CustomPropertyDrawer(typeof(TriggerCondition), true)]
    public class ConditionTriggerDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (EditorGUI.DropdownButton(typeRect, new GUIContent(displayName ?? "Select Trigger Condition Type"), FocusType.Keyboard))
            {
                var menu = new GenericMenu();
                if (typeMap == null || typeMap.Count == 0)
                {
                    menu.AddDisabledItem(new GUIContent("No Trigger Condition available"));
                    menu.ShowAsContext();
                    return;
                }
        ...
        {
            var baseType = typeof(TriggerCondition);
